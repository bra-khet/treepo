//! Table-driven trigonometry — `F-SKEL-6`, the mitigation for `RISK-2`.
//!
//! > "Deterministic trigonometry. Turtle interpretation must not depend on platform
//! > `libm`: use a fixed lookup table with deterministic interpolation, or a pinned
//! > software implementation." — PRD `F-SKEL-6`
//!
//! `sin` and `cos` are not guaranteed to be bit-identical across platform math libraries.
//! They are usually close, and "usually close" is the worst possible failure mode: a tree
//! that is subtly the wrong shape on someone else's machine, with no error to bisect. This
//! module removes the possibility rather than testing for its symptoms.
//!
//! # How it works
//!
//! A 1025-entry table covers a quarter turn in Q30. The other three quadrants come from
//! reflection and negation, which are exact — so the sign and mirror symmetries of sine
//! hold to the bit, not to a tolerance. The 20 low bits of the [`Angle`] that the table
//! does not resolve drive a linear interpolation between neighbouring entries.
//!
//! Everything is integer arithmetic. There is no float anywhere in this module, so there
//! is nothing for a platform to round differently.
//!
//! # Accuracy
//!
//! Absolute error is bounded by the curvature of sine across one table interval,
//! `(π/2048)² / 8`, which is under `3 × 10⁻⁷` — roughly 21 good fractional bits. At the
//! scale of a rendered tree that is a fraction of a pixel across a limb thousands of units
...
/// Entries in the quarter-turn table, covering `[0, π/2]` inclusive.
const TABLE_LEN: usize = 1025;

/// Bits of an [`Angle`] resolved by the table index; the remainder is interpolated.
const FRAC_BITS: u32 = 20;

/// Mask of the interpolated low bits.
const FRAC_MASK: u32 = (1 << FRAC_BITS) - 1;

/// A quarter turn as a raw binary angle.
const QUARTER: u32 = 0x4000_0000;

/// `sin(k · π / 2048)` in Q30, for `k` in `0..=1024`.
///
/// Generated by exact arbitrary-precision evaluation and correct rounding, not by calling
/// a platform `sin`. The values are pinned by a golden digest in this module's tests; if
/// you change them, that test is supposed to fail.
#[rustfmt::skip]
const SIN_Q30: [i32; TABLE_LEN] = [
...
/// Sine of a quarter-turn argument `x` in `0..=2^30`, returned in Q30.
const fn quarter_sin_q30(x: u32) -> i64 {
    let index = (x >> FRAC_BITS) as usize;
    if index >= TABLE_LEN - 1 {
        // Only reachable at exactly π/2, where there is nothing left to interpolate.
        return SIN_Q30[TABLE_LEN - 1] as i64;
    }
    let frac = (x & FRAC_MASK) as i64;
    let lo = SIN_Q30[index] as i64;
    let hi = SIN_Q30[index + 1] as i64;
    lo + (((hi - lo) * frac + (1 << (FRAC_BITS - 1))) >> FRAC_BITS)
}

/// Sine of an angle.
///
/// Exact at 0°, 90°, 180° and 270°, and exactly antisymmetric: `sin(-a) == -sin(a)` for
/// every representable angle.
#[must_use]
pub const fn sin(angle: Angle) -> Fx {
    let bits = angle.to_bits();
    let quadrant = bits >> 30;
    let mut x = bits & 0x3FFF_FFFF;
    if quadrant & 1 == 1 {
        // Quadrants 1 and 3 are the first quadrant mirrored about π/2.
        x = QUARTER - x;
    }
    let value = quarter_sin_q30(x);
    let signed = if quadrant >= 2 { -value } else { value };
    // Q30 -> Q32, exactly.
    Fx::from_bits(signed << 2)
}

/// Cosine of an angle.
///
/// Exactly `sin(angle + 90°)`, which makes `cos(-a) == cos(a)` hold to the bit.
#[must_use]
pub const fn cos(angle: Angle) -> Fx {
    sin(Angle::from_bits(angle.to_bits().wrapping_add(QUARTER)))
}

/// Sine and cosine together, as `(sin, cos)`.
///
/// Turtle interpretation almost always wants both; this is the call that says so.
#[must_use]
pub const fn sin_cos(angle: Angle) -> (Fx, Fx) {
    (sin(angle), cos(angle))
}
...
mod tests {
    ...
    /// Angles used by the determinism probe and by the accuracy tests. Sampled with a
    /// stride that is coprime with the table stride so that the samples land on table
    /// entries, near them, and everywhere between.
    ...
    fn table_is_well_formed() {
        ...
        assert_eq!(SIN_Q30[TABLE_LEN - 1], 1 << 30);
        for i in 0..TABLE_LEN - 1 {
            assert!(SIN_Q30[i] < SIN_Q30[i + 1], "not monotonic at {i}");
        }
    }
    ...
    fn cardinal_angles_are_exact() {
        ...
        assert_eq!(sin(Angle::QUARTER), Fx::ONE);
        assert_eq!(sin(Angle::HALF), Fx::ZERO);
        assert_eq!(sin(Angle::THREE_QUARTER), Fx::NEG_ONE);

        assert_eq!(cos(Angle::ZERO), Fx::ONE);
        assert_eq!(cos(Angle::QUARTER), Fx::ZERO);
        assert_eq!(cos(Angle::HALF), Fx::NEG_ONE);
        assert_eq!(cos(Angle::THREE_QUARTER), Fx::ZERO);
    }

    #[test]
    fn symmetries_are_exact_not_approximate() {
        // These hold to the bit because reflection and negation are exact operations on a
        // binary angle. A float implementation would only get them to a tolerance.
        for a in sampled_angles() {
            assert_eq!(sin(-a), -sin(a), "antisymmetry at {a}");
            assert_eq!(cos(-a), cos(a), "symmetry at {a}");
            assert_eq!(sin(a + Angle::HALF), -sin(a), "half-turn at {a}");
            assert_eq!(cos(a + Angle::HALF), -cos(a), "half-turn at {a}");
            assert_eq!(sin(a + Angle::QUARTER), cos(a), "phase at {a}");
        }
    }

    #[test]
    // The one place libm is allowed: as an independent check that the table is right. At
    // runtime nothing calls it — that is the entire point of this module. The result is a
    // tolerance comparison precisely because libm's answer is the one that varies.
    #[allow(clippy::float_arithmetic)]
    fn agrees_with_the_platform_libm_within_interpolation_error() {
        const TOLERANCE: f64 = 3.0e-7;
        let mut worst = 0.0f64;
        for a in sampled_angles() {
            let radians = f64::from(a.to_bits()) / 4_294_967_296.0 * core::f64::consts::TAU;
            worst = worst.max((sin(a).to_f64() - radians.sin()).abs());
            worst = worst.max((cos(a).to_f64() - radians.cos()).abs());
        }
        assert!(
            worst < TOLERANCE,
            "worst error {worst:e} exceeds {TOLERANCE:e}"
        );
    }
    ...
    fn pythagorean_identity_holds() {
        for a in sampled_angles() {
            let (s, c) = sin_cos(a);
            let unit = s * s + c * c;
            let error = (unit - Fx::ONE).abs();
            assert!(
                error < Fx::from_ratio(1, 1_000_000),
                "sin²+cos² at {a}: {unit}"
            );
        }
    }

    /// Pins the table and the interpolation. This digest is the same value
    /// `cargo xtask determinism` compares across platforms; if a refactor changes it, every
    /// tree treepo has ever generated has changed shape (`AC-DET-1`, `AC-DET-2`).
    #[test]
    fn golden_digest_of_ten_thousand_angles() {
        ...
        assert_eq!(
            ...
            "f175f604e7b1712fdac4e6fb3d113be101fb9dcca98587dc7ae34026c29faa79"
        );
    }
}
